...
export default function AdminCreatorDetailPage() {
  ...
  const verifyMut = useMutation({
    ...
    onError: (e: unknown) => toast({ title: "Action failed", description: getErrorMessage(e), variant: "destructive" }),
  });
    ...
    <AdminShell
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setLocation("/creators")} className="h-9 px-4 rounded-lg bg-card border border-border text-sm font-semibold hover-elevate">
            Back
          </button>
        </div>
      }
